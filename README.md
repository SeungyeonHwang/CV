...
**【担当フェーズ】**
    ...
    事業拡大による新規開発全般(アジャイル/スクラム)	
    製品テスト全般(テストコード作成、単体テスト〜受入テスト)
    性能改善(プロファイリング·リファクタリング·クエリチューニング)
    トラブル·シューティング
    バージョンリリース
...
- 開発・実装①
    ...
    - 【どのような機能の開発・実装か】
      - 外部クラウドサービスのログ取得のための定期的な収集モジュールおよびバージョンアップツール
        ...
        - SalesForce (SOQL)
    - 【課題・問題点】
      a. 製品の特性上、各クラウドサービスのログ取得API以外の処理が重複し、コードの重複が発生、開発工数が増加
      b. 大量のログを扱うため、DB処理がボトルネックとなり、パフォーマンスが大幅に低下
    - 【打ち手・使用した技術】
      a. ログ取得以外の部分（ユーザー認証情報、バリデーションなど）をMaven Repositoryに共通ライブラリとしてアップロードし、開発工数削減とコード重複解消を実現
      b. Visual VMなどのプロファイリングツールを使用して、ボトルネックになる処理を特定。非効率的なデータ格納をバッチ単位で処理するように変更し、無意味なクエリの乱立や繰り返しをラムダ式などでバッチ単位に分けて処理するようにリファクタリング
...
- 開発・実装②
    ...
    - 【打ち手・使用した技術】
        - 順次処理するためのフレームワークを開発
        - 例えば、1.0 -> 3.0でバージョンアップをする場合、以前は1.0 -> 3.0、2.0 -> 3.0をすべて検証しなければならなかったが、1.0 -> 2.0 -> 3.0で順次バージョンアップを行うように修正したため、テストを1回だけすればよいという利点が生まれ、検証の工数を大幅に下げることができました。
