...
## #1 フルスタックエンジニア
<u>C社【2023年08月 ～ 現在に至る】</u>

**【プロジェクト概要】**  
「声」を表現力豊かな「フォント」として変換し、安価で誰もが簡単に利用できる音声サービスを提供

**【業務内容】**  
最新の AI 音声技術を用いて、プロダクトの機能実装を推進

**【メンバー数】**  
13名

**【担当フェーズ】**
- プロダクト開発全般(設計、実装、テスト、画面作成)
- メンテナンス(テストコード作成、トラブル·シューティング)
- バージョンリリース

**【開発内容】**
- 開発・実装
    - 【概要】
      - AI 音声プラットフォームの開発: ユーザーが動画制作などで AI 音声を簡単に利用できるように機能開発
    - 【どのような機能の開発・実装か】
      - 機械学習を通じて学習された音声データをユーザーが簡単に利用できるようにバックエンドとフロントエンドを一つのストーリーとして管理し、機能を実装
        - AWS Lambda Serverless、Go(Echo)
        - Vue.js(Nuxt.js)

**【開発環境】**

    Go | Echo | AWS | Lambda | Vue.js | React.js
...
**【仕事を通して学んだこと】**
    ...
    (2) 積極性
    セールスフォースの性能検証をする時、ApexとPythonを用いて大量のログを収集できるログ収集ツールを作ったり、Visual VMをIntellijと連動してプロファイリングを行う等、今まで使われていなかった方法を提案したことがあります。
